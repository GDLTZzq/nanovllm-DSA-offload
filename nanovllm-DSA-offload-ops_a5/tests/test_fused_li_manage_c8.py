...
def output_buffers(case: C8Case) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    options = {"dtype": torch.int32, "device": case.query.device}
    source_ids = torch.empty((case.query.size(0), 1, TOPK), **options)
    destination_slots = torch.empty_like(source_ids)
    miss_counts = torch.empty((case.query.size(0),), **options)
...
def check_case(case: C8Case) -> None:
    ...
    old_pool = pool.cpu()
    source_ids, destination_slots, miss_counts = output_buffers(case)
    launch(case, pool, source_ids, destination_slots, miss_counts)
    ...
    second_sources, second_slots, second_counts = output_buffers(case)
    launch(case, pool, second_sources, second_slots, second_counts)
    torch.npu.synchronize()
    ...
    print(
        ...
        "repeat_zero_miss=1 one_device_kernel=1 caller_owned_outputs=1 ok=1",
        flush=True,
    )
...
def check_meta() -> None:
    ...
    source_ids = torch.empty((3, 1, TOPK), dtype=torch.int32, device="meta")
    destination_slots = torch.empty_like(source_ids)
    miss_counts = torch.empty((3,), dtype=torch.int32, device="meta")
    result = torch.ops.nanovllm_dsa.fused_li_manage_c8.default(
        query,
        weights,
        key,
        query_scale,
    ...
    if result is not None:
        raise AssertionError("C8 LIM caller-owned API must return None")
