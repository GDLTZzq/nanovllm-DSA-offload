...
// =====================================================================================
// [QuantLI vs LI 差异点 #13] Vector service 整体架构
// =====================================================================================
// LI (lightning_indexer_service_vector.h L45-69):
//   类: LightningIndexerServiceVector<LIT>
//   类型: Q_T, K_T, SCORE_T(uint32固定), W_T(通过TypeTraits推导)
//   无: SCALE_T, QK_T  ← LI不涉及反量化
//   ProcessVec1: 只做 weight乘+reduceSum
//   ProcessTopK: trunkLen动态(2K/4K/8K), 有returnValue分支
//
// QuantLI (本文件 L32-139):
//   类: QLIVector<QLIT>
//   类型: W_T, SCALE_T, QK_T, SCORE_T 全部从QLIType模板参数获取
//   多了: GetKeyScale() — PA/非PA反量化scale搬运
//   多了: kScaleUB_, qScaleUB_, weightFloatUB_ — 反量化专用buffer
//   ProcessVec1: 做 int32→fp32 cast + weight乘 + qScale乘 + kScale乘 + reduceSum
//                → 调用 BatchMulWeightAndReduceSum (vs LI的 MulWeightAndReduceSum)
//   ProcessTopK: trunkLen固定(6K/16K), 无returnValue
// =====================================================================================
...
public:
    ...
    using SCALE_T = typename QLIT::scaleType;   // ← QuantLI独有: scale精度
    using QK_T = typename QLIT::qkType;         // ← QuantLI独有: Cube输出类型(int32/float32)
    using SCORE_T = typename QLIT::scoreType;   // ← LI固定uint32_t, QuantLI模板化
...
// =====================================================================================
// [QuantLI vs LI 差异点 #14] ProcessVec1 — ★最核心差异★ SCORE 计算公式
// =====================================================================================
// LI (lightning_indexer_service_vector.h L279-341):
//   流水线: CV同步 → 搬weight → MulWeightAndReduceSum → 写scoreGm → 通知Cube
//   L315: MulWeightAndReduceSum(outBase,
//                               resMm1UB_[...],   ← float (Cube已出float)
//                               weightUB_[...],
//                               gSize)
//   含义: score[g] = sum_g( (Q×K^T)[g] * weight[g] )
//   只有1个MTE预取流(weight), resMm1UB_类型=float, weight layout=gSizeAlign16
//
// QuantLI (本文件 L341-448):
//   流水线: qScale预取 → kScale预取 → weight+qScale搬 → CV同步
//           → BatchMulWeightAndReduceSum → 写scoreGm → 通知Cube
//   L412-417: BatchMulWeightAndReduceSum(outBase,
//                                        qkBase,         ← QK_T (int32/float32)
//                                        weightBase, weightFloatBase,  ← weight+cast
//                                        kScaleBase,     ← Key反量化scale
//                                        qScaleBase,     ← Query反量化scale
//                                        gSize, curAivS1ProcNum)
//   含义: score[g] = sum_g( dequant(QK[g]) * weight[g] * qScale[g] * kScale[s2] )
//   3个MTE预取流(qScale/kScale/weight), resMm1UB_类型=QK_T(int32),
//   weight/qScale layout=bank优化(UB_BANK_STRIDE)
//
//   │  LI 计算公式                     │  QuantLI 计算公式                    │
//   │  score = Σ(QK_float × weight)   │  score = Σ(dequant(QK_int) ×        │
//   │                                  │      weight × qScale × kScale)       │
//   │  Cube输出: float                 │  Cube输出: int32                    │
//   │  weight: bf16→float自动          │  weight: W_T → float显式cast        │
//   │  scale: 无                       │  scale: qScale + kScale 双预取流     │
// =====================================================================================
...
__aicore__ inline void QLIVector<QLIT>::ProcessVec1(const QLICommon::RunInfo &info)
{
    ...
    if (info.isFirstS2InnerLoop) {                                  // ← QuantLI独有: qScale 和 weight 在首轮S2预取
        ...
        //qScaleGm  -->  qScaleUB_                                   ← QuantLI独有: query反量化scale
    ...
    if ((info.s2Idx - info.s2Start) % 16 == 0) {                   // ← QuantLI独有: kScale每16个S2块预取一次
        ...
        uint32_t getLen = 16 * s2BaseSize_ > (info.validS2Len - info.s2Idx * s2BaseSize_)
                                           ...
                                           : 16 * s2BaseSize_;
        //kScaleGm  -->  kScaleUB_                                   ← QuantLI独有: key反量化scale (含PA blkTable跳转)
    ...
    auto weightFloatBase = weightFloatUB_[qScalepingpong * (UB_BANK_STRIDE / sizeof(float))]; // ← QuantLI独有: weight cast to float
    auto qScaleBase = qScaleUB_[qScalepingpong * (UB_BANK_STRIDE / sizeof(SCALE_T))];         // ← QuantLI独有
    auto kScaleBase = kScaleUB_[kScalepingpong * 16 * s2BaseSize_ + ((info.s2Idx - info.s2Start) % 16) * s2BaseSize_]; // ← QuantLI独有
    ...
    // ★★★ 核心计算: LI用MulWeightAndReduceSum, QuantLI用BatchMulWeightAndReduceSum ★★★
