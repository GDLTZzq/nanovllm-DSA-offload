...
// =====================================================================================
// [QuantLI vs LI 差异点 #5] RunInfo — 每个循环迭代的运行时信息
// =====================================================================================
// LI (lightning_indexer_common.h L40-65): 18个字段
// QuantLI (本文件): 24个字段, 多了6个:
//   s2Start    — S2循环的起始位置(多核切分后每个核的S2起点)
//   validS2Len — 带attenMask时有效的S2长度
//   qScaleLoop — queryScale 的流水线 pingpong 索引
//   kScaleLoop — keyScale   的流水线 pingpong 索引
//   tensorKeyScaleOffset — keyScale GM 偏移
//
// LI 有而 QuantLI 没有的:
//   valueOutOffset — sparseValues 输出偏移 (QuantLI 不输出 value)
// =====================================================================================
struct RunInfo {
    uint32_t loop;
    uint32_t bN2Idx;
    ...
    uint32_t gS1Idx;
    uint32_t s2Idx;
    uint32_t s2Start;           // ← QuantLI 独有: S2起始位置
    uint32_t validS2Len;        // ← QuantLI 独有: attenMask下有效S2长度
    uint32_t qScaleLoop;        // ← QuantLI 独有: qScale pingpong
    uint32_t kScaleLoop;        // ← QuantLI 独有: kScale pingpong
