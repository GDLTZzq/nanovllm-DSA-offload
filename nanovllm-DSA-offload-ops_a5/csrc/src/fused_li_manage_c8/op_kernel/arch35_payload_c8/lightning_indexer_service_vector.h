    ...
    uint64_t scoreRowOffset, uint32_t validS2Len)
{
...
#else
        DataCopyPad(topkSourceIdsGm[outOffset], classifiedIndex, outputCopy);
#endif
    ...
    // BuildSortedMissPairs reuses classifiedIndex as sort scratch. Rebuild
    // the public row as sorted miss sources followed by the original hits.
    ...
    DataCopyPad(
        topkSourceIdsGm[slotOutOffset], classifiedIndex, slotOutputCopy);
