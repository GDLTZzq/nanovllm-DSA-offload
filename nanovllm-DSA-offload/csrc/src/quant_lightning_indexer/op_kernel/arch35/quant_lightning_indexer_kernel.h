...
// =====================================================================================
// [QuantLI vs LI 差异点 #8] CalcRunInfo — 偏移计算
// =====================================================================================
// LI (lightning_indexer_kernel.h L519-584):
//   签名: CalcRunInfo(loop, s2LoopIdx, runInfo)  — 3个参数, 无scale相关
//   多了 valueOutCoreOffset 计算 (L569-570) ← LI有value输出
//
// QuantLI (本文件 L534-602):
//   签名: CalcRunInfo(loop, s2LoopIdx, runInfo, qScaleLoop, kScaleLoop) — 5个参数
//   多了:
//     runInfo.qScaleLoop / kScaleLoop 赋值
//     runInfo.s2Start 赋值 (多核切分S2起点)
//     runInfo.validS2Len 赋值 (attenMask有效S2长度)
//     tensorKeyScaleOffset 计算 (L596) ← 反量化keyScale偏移
//   keyCoreOffset: LI 和 QuantLI 公式相同, 但QuantLI无prevTopkIndices不需要valueOutOffset
// =====================================================================================
template <typename QLIT>
...
// =====================================================================================
// [QuantLI vs LI 差异点 #9] ProcessBaseBlock — 每个基本块的处理 (Cube + Vector)
// =====================================================================================
// LI (lightning_indexer_kernel.h L671-684):
//   ProcessBaseBlock(loop, s2LoopIdx, runInfo)
//   CalcRunInfo(loop, s2LoopIdx, runInfo)          ← 3参数, 无qScale/kScale
//
// QuantLI (本文件):
//   ProcessBaseBlock(loop, s2LoopIdx, runInfo, qScaleLoop, kScaleLoop)  ← 5参数
//   CalcRunInfo(loop, s2LoopIdx, runInfo, qScaleLoop, kScaleLoop)       ← 5参数
//
//   双方 Pipeline 相同: Cube作Matmul → Vector作Vec1 → S2last时Vector作TopK
//   差异在于:
//     Cube ComputeMm1: 量化Matmul (int8/fp8) vs 非量化Matmul (bf16/fp16)
//     Vector ProcessVec1: 反量化+乘weight vs 只乘weight   ← ★核心★
//     Vector ProcessTopK: trunkLen固定 vs 动态, 无value输出 vs 有value输出
// =====================================================================================
template <typename QLIT>
