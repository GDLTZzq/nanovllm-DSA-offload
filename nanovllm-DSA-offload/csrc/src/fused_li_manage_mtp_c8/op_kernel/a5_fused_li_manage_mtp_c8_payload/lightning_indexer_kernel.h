 ...
 * \brief C8 MTP QuantLI 编排：每请求 2~4 个 packed query 逐行 top-2048，
 *        AIV0 在 SyncAll 后做请求级并集 + 一次性缓存更新 + 逐 query 发布 slot。
 */
...
struct TempLoopInfo {
    ...
    uint32_t actS1Size = 1U;  // 当前Batch循环处理的S1轴的实际大小（packed query 数）
...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                ...
                                __gm__ uint8_t *workspace, const LIMtpC8TilingData *__restrict tiling, TPipe *tPipe);
...
protected:
    ...
    __aicore__ inline void InitTilingData(const LIMtpC8TilingData *__restrict tilingData);
...
// =====================================================================================
// InitTilingData — Tiling常量解析（与 A5FusedLiManageC8 同源）
// =====================================================================================
// mBaseSize 由 tSize 决定：tSize<=64 时 S1_BASE_SIZE(2)*gSize（官方 QuantLI 语义），
// MTP 下 packed query 数 2~4 每请求，actS1=queryCount，gS1 按 2*query 分组。
// =====================================================================================
template <typename QLIT>
__aicore__ inline void QuantLightningIndexerKernel<QLIT>::InitTilingData(const LIMtpC8TilingData *__restrict tilingData)
{
    ...
    // 与 BF16 版 A5FusedLiManageMtp 对齐：mBaseSize = S1_BASE_SIZE(4) * gSize，
    // packed query 数 ≤ 4 时每请求恒为一个 gS1 组（4*32=128 / 4*64=256 =
    // mBaseSize），避免 2*gSize 分块把 3~4 query 请求拆成两个组、s2 块迭代
    // 与每块固定开销翻倍。
    ...
    if (constInfo.gSize == 64) {
        // Ascend 950 的 AIV UB 只有 248KB：gSize=64 时 4*gSize=256 的 M 基块
        // 需要 128KB 的 QK 缓冲，总 UB 超限（BF16 版同样超）。64 heads 回退
        // 到 2*gSize 两分组（总 UB ~214KB 可容纳，与对齐前的已验证形态一致）；
        // 32 heads 保持 4*gSize 单组。
...
__aicore__ inline void QuantLightningIndexerKernel<QLIT>::Init(__gm__ uint8_t *query,
                                              ...
                                              const LIMtpC8TilingData *__restrict tiling, TPipe *tPipe)
{
    if ASCEND_IS_AIV {
    ...
    InitActualSeqLen(actualSeqLengthsQ, candidateLens);
    ...
    // vec 把整个s2的score存储在GM，大小为s1BaseSize * kSeqSize * 2B
    ...
    if ASCEND_IS_AIV {
        ...
        // 每请求 packed query 的 top-2048 先写到 workspace 尾部（逐 query 行），
        // SyncAll 之后由 AIV0 的 FinalizeMtpRequest 消费；公共输出只有 slot 行。
        indiceOutGm.SetGlobalBuffer((__gm__ int32_t *)(workspace +
            static_cast<uint64_t>(usedCoreNum) * singleCoreScoreSize));
        ...
        candidateLensGm.SetGlobalBuffer((__gm__ int32_t *)candidateLens);
        ...
        missDestinationSlotsGm.SetGlobalBuffer((__gm__ int32_t *)missDestinationSlots);
        vectorService.InitVecInputTensor(weightsGm, qScaleGm, kScaleGm, indiceOutGm, blockTableGm,
                                         ...
                                         missSourceIdsGm, missDestinationSlotsGm,
                                         reqPoolEntriesGm, cacheTokensGm, candidateLensGm);
...
// =====================================================================================
// CalcRunInfo — 偏移计算（与 A5FusedLiManageC8 同源；MTP 的 S1 = packed query 数）
// =====================================================================================
template <typename QLIT>
        ...
        if ((tmpBlockIdx & 1U) == 0U && aiCoreIdx < usedCoreNum) {
            ...
            for (uint32_t bIdx = aiCoreIdx; bIdx < constInfo.batchSize;
                ...
                vectorService.FinalizeMtpRequest(
                    bIdx, queryBegin, queryEnd - queryBegin);
            }
        }
    }
}
...
__aicore__ inline void QuantLightningIndexerKernel<QLIT>::ProcessInvalid()
{
    if ASCEND_IS_AIV {
        ...
        uint64_t totalOutputSize = constInfo.tSize * constInfo.kHeadNum * constInfo.sparseCount;
        ...
        if (baseSize < totalOutputSize) {
            ...
            GlobalTensor<OUT_T> output = topkSlotsGm[baseSize];
...
// =====================================================================================
// ProcessBaseBlock — 每个基本块的处理 (Cube + Vector)，与 A5FusedLiManageC8 同构
// =====================================================================================
template <typename QLIT>
