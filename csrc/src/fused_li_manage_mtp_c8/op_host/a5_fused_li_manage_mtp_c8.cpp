...
#include "../op_kernel/a5_fused_li_manage_mtp_c8_workspace.h"
...
constexpr int64_t MIX_AIC_NUM = 24;   // MIX_AIC_1_2 下 AIC 数（QLI 恒满核）
    ...
    gert::TilingContext *context)
{
    ...
    const uint64_t scoreStride64 =
        static_cast<uint64_t>(MAX_QUERIES_PER_REQUEST) * sourceCapacity *
        sizeof(uint16_t);
    if (scoreStride64 > std::numeric_limits<uint32_t>::max()) {
        return ge::GRAPH_FAILED;
    }

    // 双管线运行时切换：bs<=16 -> 本 fork（arch35_ld LD 引擎，恒满 24 AIC）；
    // bs>16 -> 组长的原版管线（arch35_payload_c8，自适应核数）。切换键必须与
    // kernel 入口一致：kernel 只在 tilingData.batchSize > 16 时走组长路径。
    constexpr int64_t MAX_BATCH_OUR_PATH = 16;
    const bool useGroupLeaderPath = batch > MAX_BATCH_OUR_PATH;
    const uint32_t mixGroupCount = std::min<uint32_t>(aicCount, aivCount / 2U);
    uint32_t usedCoreNum = 0U;      // owner 数，stage2/union 语义
    uint32_t launchCoreNum = 0U;    // 发射的 MIX 组数
    uint32_t queryTileSize = 0U;
    uint32_t splitEnable = 0U;
    uint32_t scoringCoreNum = 0U;
    if (!useGroupLeaderPath) {
        // QLI LD 引擎（块池切分）恒用满 24 个 AIC：块池按块数均衡分给全部 AIC，
        // bs≤24 时每核任务量随 bs 线性下降，小 batch 也保持全核并行；LD 归并跨 AIV。
        usedCoreNum = static_cast<uint32_t>(MIX_AIC_NUM);
        launchCoreNum = usedCoreNum;
    } else {
        // 原版 host 口径：N=32（packed>64，heads=32）用单条四查询 tile/请求，
        // 否则用两条两查询 tile。bs>16 时 taskCount*2 > mixGroupCount，split 关闭，
        // 发射核数 = min(taskCount, mixGroupCount)。
        const bool useFourQueryTile =
            heads == 32 && packedQueries > 64;
        queryTileSize = useFourQueryTile ? 4U : 2U;
        const uint32_t queryTileCount =
            static_cast<uint32_t>(MAX_QUERIES_PER_REQUEST) / queryTileSize;
        const uint32_t stage1TaskCount =
            static_cast<uint32_t>(batch) * queryTileCount;
        usedCoreNum = std::min<uint32_t>(stage1TaskCount, mixGroupCount);
        const bool split =
            stage1TaskCount >= 1U && stage1TaskCount * 2U <= mixGroupCount;
        splitEnable = split ? 1U : 0U;
        launchCoreNum = split ? mixGroupCount : usedCoreNum;
        scoringCoreNum = split ? mixGroupCount : usedCoreNum;
    }
    ...
    tiling->queryTileSize = queryTileSize;
    tiling->splitEnable = splitEnable;
    tiling->scoringCoreNum = scoringCoreNum;
    // 统一用本 fork 的 superset 布局（阈值区后追加 LD 归并区）分配；组长路径只用到
    // 阈值区之前，多余尾区闲置无妨。避免 host 按路径分叉导致 entry 偏移口径不同。
    ...
    context->SetBlockDim(platform.CalcTschBlockDim(
        launchCoreNum * 2U, launchCoreNum, launchCoreNum * 2U));
    ...
    gert::InferShapeContext *context)
{
    ...
    *context->GetOutputShape(1) =
        gert::Shape({packedQueries, 1, SPARSE_COUNT});
    *context->GetOutputShape(2) = gert::Shape({packedQueries});
    *context->GetOutputShape(3) =
        gert::Shape({batch, UNION_CAPACITY});
    *context->GetOutputShape(4) =
        gert::Shape({batch, UNION_CAPACITY});
    *context->GetOutputShape(5) = gert::Shape({batch});
    *context->GetOutputShape(6) =
...
public:
    explicit A5FusedLiManageMtpC8(const char *name) : OpDef(name)
    {
        ...
        this->Output("topk_destination_slots").ParamType(REQUIRED)
            .DataType(ints).Format(formats);
        this->Output("topk_miss_count").ParamType(REQUIRED)
            .DataType(ints).Format(formats);
