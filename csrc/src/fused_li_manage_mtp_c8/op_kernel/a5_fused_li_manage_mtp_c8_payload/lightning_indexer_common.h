 ...
 * \brief C8 MTP QuantLI 公共类型（与 A5FusedLiManageC8 同源，MTP 扩展）
 */
...
// =====================================================================================
// QLIType — 每个 dtype 组合显式指定（QK 为 fp32 累加，score 为 uint16）
// =====================================================================================
...
struct QLIType {
    using queryType = Q_T;
    using keyType = K_T;
    using qkType = QK_T;         // Cube Matmul 输出类型（fp32 累加）
    ...
    using scoreType = SCORE_T;   // score 存储类型（uint16，bf16 sortable）
    using weightType = W_T;      // weight 精度（bf16）
    using scaleType = SCALE_T;   // 反量化 scale 精度（fp32）
};
...
struct RunInfo {
    ...
    uint32_t gS1Idx;
    uint32_t s2Idx;
    uint32_t s2Start;           // S2 起始位置（kScale 窗口索引 %16 用）
    uint32_t validS2Len;        // attenMask 下有效 S2 长度（kScale 预取长度用）
    uint32_t kScaleLoop;        // kScale 16 块窗口 pingpong 索引
...
// =====================================================================================
// ConstInfo — Tiling 层传递的运行时常量
// =====================================================================================
struct ConstInfo {
    // CUBE与VEC核间同步的模式
    static constexpr uint32_t QLI_SYNC_MODE4 = 4;    // 与 LI_SYNC_MODE4 值相同
    ...
    uint64_t tSize = 0ULL;             // TND 的 T 维度总大小（packed queries）
    uint64_t gSize = 0ULL;
    uint64_t qHeadNum = 0ULL;
    uint64_t kHeadNum;
    uint64_t headDim;
    uint64_t keyStride0;               // PA key GM stride
    uint64_t keyDequantScaleStride0;   // PA keyScale GM stride
    ...
    bool setL2DisableFlag = false;     // tSize≤s1BaseSize时关闭L2
