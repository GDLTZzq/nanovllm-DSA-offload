...
#include "a5_fused_li_manage_mtp_c8_qli_gl.h"
    ...
    GM_ADDR tiling)
{
    ...
    // 双管线运行时切换，键必须与 op_host 一致：bs>16 走组长原版（arch35_payload_c8，
    // 已验证 >16 大 source 全对），bs<=16 走本 fork（arch35_ld LD 引擎，已验证 ≤16）。
    // 两条管线共享同一 routePairs/thresholds/topk* 布局，随后 stage2 union 用同一套。
    if (tilingData.batchSize > 16U) {
#if C8_MTP_BISECT_PRINT
        if (GetBlockIdx() == 0U) {
            AscendC::printf("[C8BISECT] core0 entry GL stage1 bs=%u\n",
                            (uint32_t)tilingData.batchSize);
        }
#endif
        a5_fused_li_manage_mtp_c8_gl::QuantLiMtpPhase gl(
            &pipe, &tilingData);
        gl.Init(
...
#if C8_MTP_BISECT_PRINT
        if (GetBlockIdx() == 0U) {
            AscendC::printf("[C8BISECT] core0 entry LD stage1 bs=%u\n",
                            (uint32_t)tilingData.batchSize);
        }
#endif
