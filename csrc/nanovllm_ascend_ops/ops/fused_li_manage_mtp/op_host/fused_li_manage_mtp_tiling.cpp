...
ge::graphStatus FusedLiManageMtpTiling::GetTensorInfo(FusedLiManageMtpTilingInfo &tilingInfo) const
{
    ...
    op.actualSeqLengths.shape = context_->GetInputShape(ACTUAL_SEQ_K_INDEX);
    ...
    OPS_ERR_IF(op.actualSeqLengths.desc == nullptr || op.actualSeqLengths.shape == nullptr,
               OPS_LOG_E(tilingInfo.opName, "actual_seq_lengths_key desc/shape is nullptr."),
               return ge::GRAPH_FAILED);
...
ge::graphStatus FusedLiManageMtpTiling::CheckShape(FusedLiManageMtpTilingInfo &tilingInfo) const
{
    ...
    OPS_ERR_IF(tilingInfo.maxBlockNumPerBatch == 0,
               ...
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(tilingInfo.maxBlockNumPerBatch > (1U << 11),
               OPS_LOG_E(tilingInfo.opName, "block_table capacity must be <= 2048 blocks for the 18-bit source format."),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(tilingInfo.s2Size != tilingInfo.cacheSlotsSize || tilingInfo.s2Size > (1U << 18),
               OPS_LOG_E(tilingInfo.opName,
                          "cache_slots capacity must equal table capacity and be <= 2^18 tokens."),
               return ge::GRAPH_FAILED);
...
ge::graphStatus FusedLiManageMtpTiling::DoTiling(FusedLiManageMtpTilingInfo *tilingInfo)
{
    ...
    if (mtp_) {
        ...
        constexpr uint32_t MTP_UNION_CAPACITY = MTP_ROUTES * DECODE_SPARSE_COUNT;
        constexpr uint32_t MTP_THRESHOLD_STRIDE = 8U;
        const uint64_t metadataBatch = tilingInfo->bSize / MTP_ROUTES;
        // Keep this order synchronized with MtpWorkspace::{Pair0Offset,
        // Pair1Offset, ScoreOffset, ThresholdOffset} in the kernel.
        workspaceSize += metadataBatch * MTP_UNION_CAPACITY * sizeof(float) * 2U;
