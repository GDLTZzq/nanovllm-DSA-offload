...
constexpr uint32_t MTP_ROUTES = 4U;
    ...
    int64_t outputOffset, bool hasLongIndexTag)
{
    ...
    // The hit/miss ordering is already deterministic here: misses form the
    // prefix and hits form the suffix.  Publish the final topk_src_ids
    // semantics directly from the LI owner so the later cache-update owner
    // never has to rewrite a complete 2048-token source row.  The slot row was
    // copied above, so it can now serve as the all--1 select input.
    LocalTensor<uint8_t> missMask = scratchLocal.template ReinterpretCast<uint8_t>();
    CompareScalar(missMask, slotLocal, constInfo_.INVALID_IDX,
                  CMPMODE::EQ, constInfo_.sparseCount);
    PipeBarrier<PIPE_V>();
    Duplicate(slotLocal, constInfo_.INVALID_IDX, constInfo_.sparseCount);
    PipeBarrier<PIPE_V>();
    Select(indexLocal.template ReinterpretCast<float>(), missMask,
           indexLocal.template ReinterpretCast<float>(),
           slotLocal.template ReinterpretCast<float>(),
           SELMODE::VSEL_TENSOR_TENSOR_MODE, constInfo_.sparseCount);
    PipeBarrier<PIPE_V>();
}
...
__aicore__ inline void LIVector<LIT>::ProcessVec(const LICommon::RunInfo &info)
{
    ...
    for (int innerS1Idx = 0; innerS1Idx < cuS1ProcNumPerAiv; innerS1Idx++) {
        ...
        if (cuRealAcSeq > 0 && cuS2Len > 0) {
            ...
            // Publish the immutable reduction result before the TopK path
            // reuses its UB.  indiceOutOffset supplies the distributed global
            // request/route address used by both direct output and LD reduce.
            ...
            // The next inner-S1 iteration overwrites reduceOutInner.
            SetWaitFlag<HardEvent::MTE3_V>(HardEvent::MTE3_V);
            ...
            if (needCopyOutGm) {
                if (!constInfo_.returnValue) {
                    ...
                    LocalTensor<float> valueULocal = outQueue_.AllocTensor<float>();
                    // Publish the threshold through MTE3 instead of a scalar
                    // GM store.  The eviction owner can be a different AIV;
                    // SyncAll orders the cores but does not flush a producer's
                    // scalar data cache for a consumer-side scalar GetValue.
                    ...
                    DataCopyPad(
                        thresholdScratchGm[
                            (static_cast<uint64_t>(info.bIdx) * MTP_ROUTES +
                             static_cast<uint32_t>(cuS1Idx)) * MTP_THRESHOLD_STRIDE],
                        valueULocal,
...
__aicore__ inline void LIVector<LIT>::ProcessLD()
{
    ...
    for (uint32_t innerS1Idx = s1LdStartIdx; innerS1Idx < s1LdStartIdx + s1VecNum; innerS1Idx++) {
        ...
        if (!constInfo_.returnValue) {
            ...
            // Use DMA publication and wait for it before reusing outValueUb,
            // so the following outer SyncAll has a completed GM write to
            // order rather than a dirty scalar-cache line.
            outValueUb.SetValue(
