...
constexpr uint64_t UNION_CAPACITY = 8192U;
...
{
    return Pair0Offset(blockNum, headNum) + batch * UNION_CAPACITY * sizeof(float);
}
...
{
    return Pair1Offset(blockNum, headNum, batch) + batch * UNION_CAPACITY * sizeof(float);
}
